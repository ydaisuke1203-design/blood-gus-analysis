(function () {
  ...
  function respirationAnalysis(pao2, paco2, fio2) {
    ...
    if (pf < 300) {
      out.push(paco2 <= 45 ? "1型呼吸不全" : "2型呼吸不全");
    }
  ...
  // Boston approachは「改行問題の切り分け」のため、まずヘッダだけ出す
  // （ここに本体ロジックを追加するのは次のステップに回す）
  function bostonPlaceholder() {
    return ["", "【Boston_approach】", "（未実装）"];
  }
  ...
  function runAnalysis() {
    ...
    const lines = [];
    lines.push("【Respiration analysis】");
    lines.push(...respirationAnalysis(pao2, paco2, fio2));
    lines.push(hr());
    lines.push(...anionGap(hco3, na, k, cl, p, alb));
    lines.push(...bostonPlaceholder());

    // ここが最重要：必ず "\n"
    setText("out1", lines.join("\n"));

    setText("out2", ""); // Stewartは一旦空
  }
